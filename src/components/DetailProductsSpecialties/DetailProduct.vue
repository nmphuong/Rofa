<template>
  <div class="dtp-cpn py-3 container">
    <div class="row p-0 m-0" v-if="dataDetailProduct !== null">
      <div class="col-md-6">
        <Images :images="dataDetailProduct.images"/>
      </div>
      <div class="col-md-6">
        <InfoProduct :dataProduct="dataDetailProduct" />
      </div>
      <div class="col-12 p-0 py-2 m-0">
        <hr />
      </div>
      <div class="col-12">
        <Description :description="dataDetailProduct.description" />
      </div>
...
export default {
  ...
  methods: {
    ...
    async getDetailProductsSpecialties () {
      ...
      await this.$store.dispatch('products/getDetailProductsSpecialties', this.id).then(async (result) => {
