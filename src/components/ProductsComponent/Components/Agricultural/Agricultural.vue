<template>
  <div class="row p-0 m-0" v-if="dataAgricultural !== null">
    <b-link :to="'/products/' + data.slug" v-for="(data, index) in dataAgricultural" :key="index" class="Agricultural-component col-lg-3 col-md-4 col-sm-4 col-6 p-0 m-0 text-dark">
      <div class="w-100">
  ...
  methods: {
    ...
    async handleGetAgricultural () {
      ...
      await this.$store.dispatch('home/getProductHome', this.query).then(async (result) => {
        await this.$store.dispatch('home/getBanners', 7).then(async (banner) => {
